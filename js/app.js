...
function loadFeedList() {
  ...
  doAPIRequest("getfeeds",{feed:0},function(data) { //success
    ...
    data.items.forEach(function(e){
      ...
      //settings
      el=$($("#tpl-settings-feed").jqote(obj));
...
jQuery(document).ready(function($){
  ...
  $(window).bind("hashchange",function() {
    ...
    switch(op[1]) {
      ...
      case "settings":
        $("#opsettings").show();
//        $("#settings-feeds tr").draggable({containment:"#settings-feeds tbody",handle:"td"});
        break;
    }
    appstate.view=op[1];
  });
